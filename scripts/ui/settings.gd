...
# =========================================================
# ÉCHELLE D'INTERFACE — APPLICATION IMMÉDIATE (§8.133, révisé §8.134.2)
# =========================================================
# ⚠️⚠️ LA CONFIRMATION À REBOURS DE 10 s A ÉTÉ RETIRÉE — elle était un DÉFAUT, pas une sécurité.
# Le patron « appliquer puis revenir sans confirmation » suppose que le joueur regarde le dialogue.
# Ici il regarde exactement l'inverse : il change la TAILLE DE L'INTERFACE pour observer la BARRE DE
# NAVIGATION, en haut de l'écran, pendant que le dialogue attend au centre. Au bout de 10 s son
# choix était donc annulé EN SILENCE — et `set_comfort` le réécrivait sur disque. Symptôme vécu :
# « la navbar est propre à tous les pourcentages, mais dès que je quitte les Paramètres l'écriture
# WASTELAND WARFARE revient ». Reproduit et mesuré : après le clic `ui_scale = 1.3`, après 12 s
# d'inaction `ui_scale = 1.0`.
#
# Ce que la confirmation prétendait couvrir est DÉJÀ tenu, et mieux, par deux mécanismes qui
# n'existaient pas quand ce patron a été spécifié :
#   • les paliers que la fenêtre ne peut pas porter sont GRISÉS (`ui_scale_fits`) — on ne peut donc
#     plus choisir une échelle qui ne rentre pas ;
#   • la nav se DÉGRADE et garantit ⚙ et ⏻ à l'écran à toutes les échelles (§8.133, matrice 12/12).
# Le joueur peut donc TOUJOURS revenir en arrière lui-même : il n'y a plus rien à protéger, et un
# réglage qui se défait tout seul est bien pire qu'un réglage qu'on doit changer deux fois.
#
# ⚠️ Les clés i18n `SETTINGS_SCALE_KEEP` / `_REVERT_IN` / `_REVERT` / `_CONFIRM` / `_REVERTED`
# restent au CSV : on ne supprime JAMAIS une clé (règle projet), elles deviennent simplement
# orphelines. `SETTINGS_SCALE_TOO_BIG`, elle, sert toujours (mention des paliers grisés).
